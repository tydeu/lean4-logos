...
class ForallNatIntro (L : Logic P) (N : PNat P T) (FaN : SForallNat P T) :=
  toFun : (f : T -> P) -> ((a : T) -> (L |- nat a) -> (L |- f a)) -> 
    (L |- forallNat a => f a)

def forallNatIntro  {L : Logic P} [N : PNat P T] [FaN : SForallNat P T] 
  [K : ForallNatIntro L N FaN] {f} := K.toFun f

class ForallNatElim (L : Logic P) (N : PNat P T) (FaN : SForallNat P T) := 
  toFun : (f : T -> P) -> (L |- forallNat a => f a) ->
    ((a : T) -> (L |- nat a) -> (L |- f a))

def forallNatElim   {L : Logic P} [N : PNat P T] [FaN : SForallNat P T] 
...
[N : PNat P T] [Fa : SForall P T] [larr : LArr P]
