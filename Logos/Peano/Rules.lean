...
-- Axiom 1
class NatZero (L : Logic P) (N : PNat P T) (Z : Zero T) :=
  toFun : L |- nat (0 : T)

abbrev natZero (L : Logic P) (T : Sort v) [N : PNat P T] [Z : Zero T] 
  [K : NatZero L N Z] := K.toFun

abbrev nat0 {L : Logic P} [N : PNat P T] [Z : Zero T] 
  [K : NatZero L N Z] := K.toFun

-- Axiom 6
class NatSuccNat (L : Logic P) (N : PNat P T) (S : Succ T) :=
  toFun : (n : T) -> (L |- nat n) -> (L |- nat (S n))

abbrev natSuccNat {L : Logic P} [N : PNat P T] [S : Succ T] 
  [K : NatSuccNat L N S] {n} := K.toFun n

abbrev natS {L : Logic P} [N : PNat P T] [S : Succ T] 
  [K : NatSuccNat L N S] {n} := K.toFun n

-- Axiom 7a
class EqNatToEqSucc (L : Logic P) (N : PNat P T) (Q : SEq P T) (S : Succ T) :=
  toFun : (m n : T) -> (L |- nat m) -> (L |- nat n) -> 
    (L |- m = n) -> (L |- S m = S n)

abbrev eqNatToEqSucc {L : Logic P} [N : PNat P T] [Q : SEq P T] [S : Succ T]
  [K : EqNatToEqSucc L N Q S] {m n} := K.toFun m n

instance iEqNatToEqSuccOfFApplyT
  {L : Logic P} [N : PNat P T] [Q : SEq P T] [S : Succ T]
...
-- Axiom 7b
class EqSuccToEqNat (L : Logic P) (N : PNat P T) (Q : SEq P T) (S : Succ T) :=
  toFun : (m n : T) -> (L |- nat m) -> (L |- nat n) -> 
    (L |- S m = S n) -> (L |- m = n)

abbrev eqSuccToEqNat {L : Logic P} [N : PNat P T] [Q : SEq P T] [S : Succ T]
...
class SuccNatEqZeroFalse (L : Logic P) 
  (N : PNat P T) (Q : SEq P T) (Z : Zero T) (S : Succ T) :=
