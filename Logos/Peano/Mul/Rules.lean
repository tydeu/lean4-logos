...
class MulNatZeroEqZero (L : Logic P) 
  (N : SNat P T) (Q : SEq P T) (M : SMul T) (Z : Zero T)  :=
...
class MulNatSuccEqAddMul (L : Logic P) 
  (N : SNat P T) (Q : SEq P T) (M : SMul T) (A : SAdd T) (S : Succ T) := 
...
class MulNatOneEqNat (L : Logic P)
  (N : SNat P T) (Q : SEq P T) (M : SMul T) (O : One T) :=
...
class MulNatAddEqAddMul (L : Logic P)
  (N : SNat P T) (Q : SEq P T) (M : SMul T) (A : SAdd T) := 
  toFun : (a b c : T) -> 
    (L |- nat a) -> (L |- nat b) -> (L |- nat c) ->
    (L |- a * (b + c) = (a * b) + (a * c))
...
abbrev eqNatMulNatLeft' {L : Logic P} 
  [N : SNat P T] [Q : SEq P T] [M : SMul T] [K : EqNatMulNatLeft L N Q M] 
...
abbrev eqNatMulNatRight' {L : Logic P} 
  [N : SNat P T] [Q : SEq P T] [M : SMul T] [K : EqNatMulNatRight L N Q M] 
  {c a b} (Nc Na Nb) := K.toFun a b c Na Nb Nc
...
class NatMulZero (L : Logic P) (N : SNat P T) (M : SMul T) (Z : Zero T) :=
  toFun : L |- nat (0 * 0 : T)

abbrev natMulZero {L : Logic P} [N : SNat P T] [M : SMul T] [Z : Zero T]
  [K : NatMulZero L N M Z] := K.toFun

-- nat (a * 0)

class NatMulNatZero (L : Logic P) (N : SNat P T) (M : SMul T) (Z : Zero T) :=
  toFun : (a : T) -> (L |- nat a) -> (L |- nat (a * 0))

abbrev natMulNatZero {L : Logic P} [N : SNat P T] [M : SMul T] [Z : Zero T]
  [K : NatMulNatZero L N M Z] {a} := K.toFun a

-- nat (0 * a)

class NatMulZeroNat (L : Logic P) (N : SNat P T) (M : SMul T) (Z : Zero T) :=
  toFun : (a : T) -> (L |- nat a) -> (L |- nat (0 * a))

abbrev natMulZeroNat {L : Logic P} [N : SNat P T] [M : SMul T] [Z : Zero T]
  [K : NatMulZeroNat L N M Z] {a} := K.toFun a 

-- nat (a * b)

class NatMulNat (L : Logic P) (N : SNat P T) (M : SMul T) :=
  toFun : (a b : T) -> (L |- nat a) -> (L |- nat b) -> (L |- nat (a * b))

abbrev natMulNat {L : Logic P}[N : SNat P T] [M : SMul T] 
  [K : NatMulNat L N M] {a b} := K.toFun a b

abbrev natMul {L : Logic P} [N : SNat P T] [M : SMul T] 
  [K : NatMulNat L N M]{a b} := K.toFun a b
